...
# Pre-fitted Gabor wavelet parameters defining the incident ultrasonic pulse
GABOR_PARAMS = {
    'A': -0.64956,   
    'sigma': -0.085,
    'omega': 4.7e7, 
...
def train():
    ...
    # 2. Directory Management for Experiment Tracking
    ...
    print(f"=== START TRAINING: {run_name} (Curriculum Learning Strategy) ===")

    # 3. Data Ingestion & Physics Engine Initialization
    ...
    with open(os.path.join(run_dir, "log.csv"), "w", newline='') as csv_file:
        ...
        for epoch in range(1, CONFIG.train_epochs + 1):
            ...
            if epoch <= 500:
                # Phase 1: Kickstart. SpeedNet is frozen to the baseline (1450 m/s). 
                # WaveNet learns the basic physics of wave propagation first.
                phase = "Kickstart (Frozen)"
                w_pde, w_data, w_ic = 10.0, 1.0, 0.001
                if not speednet_frozen:
                    for param in pinn_model.speed_net.parameters(): 
                        param.requires_grad = False
                    speednet_frozen = True
            
            elif epoch <= 2000:
                # Phase 2: Injection. SpeedNet is unfrozen. Data fidelity weight linearly 
                # increases to gradually enforce boundary measurements.
                ...
                # Phase 3: Refinement. High data weight to resolve fine spatial details.
                phase = "Refinement"
            ...
            # Track gradient norms to monitor network balancing
            ...
            # Checkpointing strategy: save best physical model and regular intervals
